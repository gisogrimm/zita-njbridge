...
static struct option options[] = {
    ...
    {"chan", 1, 0, CHAN},   {"mtu", 1, 0, MTU},     {"hops", 1, 0, HOPS},
    {"16bit", 0, 0, BIT16}, {"24bit", 0, 0, BIT24}, {"float", 0, 0, FLT32},
    {"ipv4", 0, 0, IPV4},   {"ipv6", 0, 0, IPV6},   {0, 0, 0, 0}};
...
static int opensocket(Sockaddr* A)
{
    ...
    {
        if(dev_arg)
            fd = sock_open_mcsend(A, dev_arg, 1, hops_arg);
        else
        {
...
int main(int ac, char* av[])
{
    ...
    ppper =
        Netdata::packetsperperiod(psize, jacktx->bsize(), form_arg, chan_arg);
    ...
    descpack.init_audio_desc(0, form_arg, chan_arg, psize, jacktx->fsamp(),
                             jacktx->bsize());
